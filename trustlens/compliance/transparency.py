...
def generate_privacy_notice(profile: TenantComplianceProfile) -> str:
    ...
    return (
        ...
        f"- Categories of data subjects: " + ", ".join(profile.categories_of_subjects or ["(TBD)"]) + "\n\n"
        f"## Why we process it\n"
        f"- Purposes: " + ", ".join(profile.purposes_of_processing or ["service delivery"]) + "\n"
        ...
        f"- Transfer basis: " + ", ".join(profile.cross_border_basis or ["SCC / adequacy"]) + "\n\n"
        f"## Automated decision-making (AI)\n"
        f"This service uses TrustLens to verify AI outputs. Every response is\n"
        f"accompanied by a signed certificate stating which claims are grounded\n"
        f"and which are flagged. You may request human review of any AI-assisted\n"
        f"decision via the contact above (GDPR Art.22).\n"
    )
